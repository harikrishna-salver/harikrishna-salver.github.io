...
const getClientSideResponse = (userQuery: string): string => {
  ...
  if (query.includes("ai") || query.includes("gpt") || query.includes("openai") || query.includes("summariz") || query.includes("reviewer") || query.includes("cursor")) {
  ...
  if (query.includes("react") || query.includes("frontend") || query.includes("architecture") || query.includes("stack") || query.includes("technology") || query.includes("expert") || query.includes("type")) {
  ...
  if (query.includes("ci/cd") || query.includes("jenkins") || query.includes("build") || query.includes("automation") || query.includes("pipeline") || query.includes("speed") || query.includes("action")) {
  ...
  if (query.includes("accessibility") || query.includes("wcag") || query.includes("screen reader") || query.includes("aria") || query.includes("player") || query.includes("jaws")) {
  ...
  if (query.includes("contact") || query.includes("hire") || query.includes("email") || query.includes("phone") || query.includes("reach") || query.includes("linkedin")) {
  ...
  if (query.includes("company") || query.includes("experience") || query.includes("work") || query.includes("history") || query.includes("skillsoft") || query.includes("egnify") || query.includes("tazzo")) {
...
export default function AIChatTwin() {
  ...
  const handleSendMessage = async (textToSend: string) => {
    ...
    try {
      // In static hosts (like Github Pages), we skip the API request if we already know we are in static mode
      if (isStaticMode) {
        throw new Error("Static serverless fallback active");
      }

      // We pass the full historical messages list for conversational memory!
      ...
      console.warn("API route unavailable. Seamlessly triggering smart client-side copilot fallback.", err);
  ...
  return (
    <div className="bg-[#1a1d21] rounded-3xl p-6 border border-gray-800 shadow-xl flex flex-col h-[520px]" id="ai-twin-section">
      {/* Header */}
        ...
        {isTyping && (
          <div className="flex items-start gap-3 max-w-[80%] self-start animate-fade-in">
            ...
            <div className="bg-[#0f1113] text-slate-450 border border-gray-805 p-3 rounded-2xl rounded-tl-none flex items-center gap-2">
              ...
              <span className="text-[11px] font-mono ml-1 text-slate-500">Gemini is synthesizing a career insight...</span>
            </div>
          </div>
        )}
          ...
          <div className="flex flex-wrap gap-1.5">
            {quickPrompts.map((prompt) => (
              <button
                ...
                className="cursor-pointer text-[11px] font-sans text-left bg-[#0f1113] hover:bg-slate-800 text-slate-300 border border-gray-800 hover:border-indigo-500/30 px-3 py-1.5 rounded-xl transition-all"
              >
                {prompt}
              </button>
            ))}
